#+setupfile: setup.org
#+title: Proving type inequalities in Coq
#+date: <2016-05-29>
#+excerpt: A trick on proving inequalities of types with different numbers of inhabitants.
...
Both =f true= and =f false= are evaluated to =tt=.
There is not much choice here.
However we know, that $\forall x, (g \cdot f) \ x = x$ .
By instantiating =x= with =true= and =false= we get $g (f \ true) = true$ and $g (f \ false) = false$.
