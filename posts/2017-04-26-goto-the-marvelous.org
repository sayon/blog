#+setupfile: setup.org
#+title: The virtues of using goto
#+date: <2017-04-26>
#+excerpt: Using <code>goto</code> is marginalized in programmers community which likes the simple one-line recipes for the complex challenges of writing code. In some situations, however, using <code>goto</code> is the best tool in your box, and I will show you why.
#+filetags: :education:programming:goto:c:
...
devoted supporters was [[https://en.wikipedia.org/wiki/Edsger_W._Dijkstra][Edsger W. Dijkstra]].
...
harmful is that it is usually paired with /assignments/.
...
If we want to implement a [[https://en.wikipedia.org/wiki/Finite-state_machine][Finite State Machine (FSM)]], then =goto= ’s are
...
Here is a toy example, taken from [my book](http://www.apress.com/us/book/9781484224021). This FSM checks whether
...
technique called /model checking/ which allows you to reason about the program
...
[[http://spinroot.com/spin/Man/Exercises.html][exercise page]] for SPIN model checker.
...
Computed =goto=  is a non-standard feature supported by many popular C and C++
...
than 30% of the execution time, and computed =goto= allows one to speed it up.
...
- It is forced to perform the bounds check according to C standard. If no
  ...
  well, no part of the =switch= body is executed[fn:1].
  Computed =goto= will just result in an undefined behavior in case of an invalid opcode.
...
so that =switch= is predicted better, so the performance gain from using
computed =goto= is not that substantial.
...
[fn:1] C99 standard, Section 6.8.4.2.
